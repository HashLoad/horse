...
# GET /ping
res=$(curl -s http://localhost:9086/ping)
assert_response "GET /ping" "$res" "pong"

# GET /resource/:id
res=$(curl -s -H "authorization: Bearer JWT_123" "http://localhost:9086/resource/abc?q=busca")
assert_response "GET /resource/:id (Params, Query, Header)" "$res" "abc"
assert_response "GET /resource/:id (Query)" "$res" "busca"
assert_response "GET /resource/:id (Header)" "$res" "Bearer JWT_123"

# POST /resource
res=$(curl -s -X POST -H "Content-Type: text/plain" -d "Ola_Horse" http://localhost:9086/resource)
assert_response "POST /resource (Body)" "$res" "POST OK: Ola_Horse"

# PUT /resource/:id
res=$(curl -s -X PUT http://localhost:9086/resource/123)
assert_response "PUT /resource/:id" "$res" "Updated"

# PATCH /resource/:id
res=$(curl -s -X PATCH http://localhost:9086/resource/123)
assert_response "PATCH /resource/:id" "$res" "Patched"

# DELETE /resource/:id
res=$(curl -s -X DELETE http://localhost:9086/resource/123)
assert_response "DELETE /resource/:id" "$res" "Deleted"

# QUERY /search
res=$(curl -s -X QUERY -d "filtro_busca" http://localhost:9086/search)
assert_response "QUERY /search" "$res" "SEARCH RESULT FOR: filtro_busca"

# POST /upload (Multipart)
echo "Conteudo de amostra de upload" > temp_sample_upload.txt
res=$(curl -s -X POST -F "file=@temp_sample_upload.txt" http://localhost:9086/upload)
assert_response "POST /upload (Multipart)" "$res" "Upload OK"
rm -f temp_sample_upload.txt

# GET /error-trigger (Clean JSON Exception)
res=$(curl -s -i http://localhost:9086/error-trigger)
