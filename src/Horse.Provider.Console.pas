...
begin
  Result := not Assigned(FIdHTTPWebBrokerBridge);
end;

class procedure THorseProvider.OnQuerySSLPort(APort: Word; var VUseSSL: Boolean);
begin
  VUseSSL := Assigned(FHorseProviderIOHandleSSL) and FHorseProviderIOHandleSSL.Active;
end;
...
begin
  if not Assigned(FHorseProviderIOHandleSSL) then
...
begin
  ...
  try
    ...
    if Assigned(FHorseProviderIOHandleSSL) then
